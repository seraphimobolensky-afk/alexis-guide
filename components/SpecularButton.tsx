...
export interface SpecularButtonProps {
  ...
  /** Adaptation for this app: lets a wrapper know whether WebGL actually
   * initialized, so it can swap to a plain styled button instead of guessing. */
...
const SpecularButton = ({
  ...
  }, [onStatusChange, radius, lineColor, baseColor, intensity, shineSize, shineFade, thickness, speed, followMouse, proximity, autoAnimate]);
  ...
  useEffect(() => {
    ...
    // A calm, non-animated frame used both for the initial paint and for the
    // whole session when prefers-reduced-motion is on — no loop, no drift.
    const renderStatic = () => {
      if (contextLost) return;
      const p = propsRef.current;
      lineC.set(p.lineColor);
      baseC.set(p.baseColor);
      program.uniforms.uAngle.value = STATIC_ANGLE;
      program.uniforms.uRadius.value = Math.min(p.radius, Math.min(sizeRef.w, sizeRef.h) / 2) * dpr;
      program.uniforms.uLineColor.value = [lineC.r, lineC.g, lineC.b];
      program.uniforms.uBaseColor.value = [baseC.r, baseC.g, baseC.b];
      program.uniforms.uIntensity.value = p.intensity * (p.autoAnimate ? 1 : 0.55);
    ...
    // back to a slow sweep when the pointer hasn't moved yet. Inert (never
    // read) while reduceMotion is on, since the static path never consults it.
